...
export default class PreInvoiceDao {
    ...
    async getAll(conditions) {
        ...
        try {
            let query = db('pre_invoices')
                ...
                .select('stock_items.ID', 'stock_items.product_size_id', 'stock_items.width', 'stock_items.parent_id', 'stock_items.single_product_code', 'stock_items.status as stock_item_status', 'pre_invoices.ID', 'pre_invoices.stock_item_id', 'pre_invoices.status as pre_invoice_status', 'pre_invoices.price', 'pre_invoices.weight', 'pre_invoices.number', 'pre_invoices.customer_name', 'pre_invoices.width', 'sp.shop_id')
    ...
    async createPending(markedItem) {
        // using transaction knexjs
        try {
            const [insertId] = await db('pre_invoices')
                .insert(markedItem);
            return {
                ID: insertId,
                ...markedItem
    ...
    async updatePending(pendingItems, soldItems) {
        // using transaction knexjs
        return await db.transaction(async (trx) => {
            const updatedPreInvoiceItems = [];
            ...
            try {
                ...
                if (soldItemIds.length > 0 && pendingItemIds.length > 0) {
                    const result = await trx('pre_invoices')
